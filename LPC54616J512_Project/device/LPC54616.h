...
**
**     Abstract:
**         CMSIS Peripheral Access Layer for LPC54616
**
 ...
 * @version 1.2
 * @date 2017-06-08
 * @brief CMSIS Peripheral Access Layer for LPC54616
 *
 * CMSIS Peripheral Access Layer for LPC54616
 */

#ifndef _LPC54616_H_
#define _LPC54616_H_                             /**< Symbol preventing repeated inclusion */
...
#include "system_LPC54616.h"           /* Device specific configuration file */
...
typedef enum _dma_request_source
{
    ...
    kDmaRequestDMIC0                = 16U,         /**< Digital microphone interface 0 */
    kDmaRequestDMIC1                = 17U,         /**< Digital microphone interface 1 */
    kDmaRequestSPIFI                = 18U,         /**< SPI Flash Interface */
    kDmaRequestSHA                  = 19U,         /**< Reserved */
    ...
    kDmaRequestNoDMARequest28       = 28U,         /**< No DMA request 28 */
    kDmaRequestNoDMARequest29       = 29U,         /**< No DMA request 29 */
...
/*! ENABLE - FIFO enable.
 *  0b0..FIFO is not enabled. Enabling a DMIC channel with the FIFO disabled could be useful while data is being streamed to the I2S, or in order to avoid a filter settling delay when a channel is re-enabled after a period when the data was not needed.
 ...
 *  0b0..SCK LOW. The SPIFI drives SCK low after the rising edge at which the last bit of each command is captured, and keeps it low while CS is HIGH.
 *  0b1..SCK HIGH. the SPIFI keeps SCK high after the rising edge for the last bit of each command and while CS is HIGH, and drives it low after it drives CS LOW. (Known serial flash devices can handle either mode, but some devices may require a particular mode for proper operation.) MODE3, RFCLK, and FBCLK should not all be 1, because in this case there is no final falling edge on SCK on which to sample the last data bit of the frame.
 */
 ...
 *  0b1..Feedback clock. Read data is sampled using a feedback clock from the SCK pin. This allows slightly more time for each received bit. MODE3, RFCLK, and FBCLK should not all be 1, because in this case there is no final falling edge on SCK on which to sample the last data bit of the frame.
 */
