...
USB_DMA_DATA_NONCACHEABLE static uint8_t s_dataRxBuffer[ENET_RXRTCSBUFF_NUM][ENET_FRAME_MAX_FRAMELEN + RNDIS_USB_OVERHEAD_SIZE +
                                                   sizeof(enet_header_t)] ;
...
void VNIC_EnetCallback(pbuf_t *pbuffer)
{
    ...
    {
        g_deviceComposite->cdcVnic.nicTrafficInfo.enetRxEnet2usb++;
    }
    return;
}
...
usb_status_t VNIC_EnetTxDone(void)
{
    ...
    if (kStatus_USB_Success == error)
    {
        ...
        g_deviceComposite->cdcVnic.nicTrafficInfo.enetTxUsb2enetSent++;
...
usb_status_t VNIC_EnetSend(uint8_t *buf, uint32_t len)
{
    ...
    pbuf_t packetBuffer;
    vnic_enet_transfer_t cdcAcmTransfer;
    //g_deviceComposite->cdcVnic.nicTrafficInfo.enetTxHost2usb++;
    packetBuffer.payload = VNIC_EnetTxBufAlloc();
    if (NULL == packetBuffer.payload)
    {
        sts = kStatus_USB_Busy;
        g_deviceComposite->cdcVnic.nicTrafficInfo.enetTxHost2usbDiscard++;
    ...
    else
    {
        ...
        if (ENET_OK != error)
        {
            //g_deviceComposite->cdcVnic.nicTrafficInfo.enetTxUsb2enetFail++;
            VNIC_EnetTxBufFree(&packetBuffer);
        ...
        else
        {
            ...
            if (kStatus_USB_Success != sts)
            {
                ...
                usb_echo("gVNIC_EnetQueuePut failed\n");
                return sts;
            }
            g_deviceComposite->cdcVnic.nicTrafficInfo.enetTxUsb2enet++;
        }
    }
...
usb_status_t VNIC_EnetClearEnetQueue(void)
{
    ...
    while (kStatus_USB_Success == error)
    {
        ...
        if (kStatus_USB_Success == error)
        {
            ...
            g_deviceComposite->cdcVnic.nicTrafficInfo.enetRxUsb2hostCleared++;
        }
    }
    /* Clear Tx queue */
    ...
    while (kStatus_USB_Success == error)
    {
        ...
        if (kStatus_USB_Success == error)
        {
            ...
            g_deviceComposite->cdcVnic.nicTrafficInfo.enetTxUsb2hostCleared++;
        }
    }
    return error;
}
